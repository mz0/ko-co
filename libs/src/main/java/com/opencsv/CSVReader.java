...
import static com.exactpro.uncommon.StringUtils.abbreviate;
...
public class CSVReader implements Closeable, Iterable<String[]> {
    ...
    protected static final List<Class<? extends IOException>> PASSTHROUGH_EXCEPTIONS = List.of(
        CharacterCodingException.class, CharConversionException.class,
     ...
     */
    CSVReader(Reader reader, int line, ICSVParser csvParser, boolean keepCR, boolean verifyReader, int multilineLimit,
    ...
    private void primeNextRecord() throws IOException {
        ...
        do {
            ...
            if (!hasNext) {
                if (parser.isPending()) {
                    throw new CsvMalformedLineException(String.format("unterminated quote in line %d: %s",
                        lastSuccessfulLineRead, abbreviate(parser.getPendingText(), MAX_WIDTH)), parser.getPendingText());
                }
                return;
            }
            ...
            if (multilineLimit > 0 && linesInThisRecord > multilineLimit) {
                ...
                String messageFormat = "Encountered single record" +
                 " with more lines than the specified upper limit of %d (row %d). Context: %s.";
                String message = String.format(messageFormat, multilineLimit, row, context);
                throw new CsvMultilineLimitBrokenException(message, row, parser.getPendingText());
            }
     ...
     * <p>
     * By default, the value is true, which is the functionality for OpenCSV 3.0 (2014).
