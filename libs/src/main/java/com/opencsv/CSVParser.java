...
import com.exactpro.uncommon.StringUtils;

import java.io.IOException;
import java.util.ArrayList;
import java.util.List;

import static com.exactpro.uncommon.NullAware.contains;
...
public class CSVParser extends AbstractCSVParser {
    ...
    CSVParser(char separator, char quoteChar, char escape, boolean strictQuotes, boolean ignoreLeadingWhiteSpace,
        ...
        if (separator == NULL_CHARACTER) {
            throw new UnsupportedOperationException("Invalid separator == '\\0'");
        }
        this.escape = escape;
    ...
    protected String convertToCsvValue(String value, boolean applyQuotesToAll) {
        ...
        boolean containsQuoteChar = contains(testValue, getQuotechar());
        boolean containsEscapeChar = contains(testValue, getEscape());
        boolean containsSeparatorChar = contains(testValue, getSeparator());
        ...
        line_done: {
            if (inQuotes && !ignoreQuotations) {
                if (multi) {
                    ...
                    throw new IOException(String.format("Unterminated quoted field at end of CSV line." +
                        " Beginning of lost text: [%s]", sfc.peekOutput()));
                }
            } else {
                inField = false;
            }
